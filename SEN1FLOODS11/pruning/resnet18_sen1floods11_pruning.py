# ╔══════════════════════════════════════════════════════════════════════╗
#  SEN1FLOODS11 — ResNet-18 LIGHTWEIGHT STUDY: Params, FLOPs & PRUNING
#  Companion to baselines/resnet18_sen1floods11_baselines.py
#
#  Team-lead request:
...
#        b) STRUCTURED L1 filter pruning — whole channels (20/40/60%)
...
def profile_model(model, img_size=224):
    ...
    macs_total = {"v": 0}
    per_layer  = {}   # name -> (macs, out_channels) for effective-FLOPs calc
    hooks = []
...
def get_baseline(run_name, samples):
    fname = f"resnet18_sen1floods11_{run_name}_best.pth"
    path  = find_weights(fname)
    torch.manual_seed(SEED)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(SEED)
    model = build_resnet18(pretrained=(path is None)).to(DEVICE)
    if path is not None:
...
def apply_structured(model, amount):
    """L1 filter pruning: zero the lowest-norm output channels of every
    conv layer (the classifier head is left intact)."""
    for name, mod in model.named_modules():
...
for run_name, samples in BASELINE_RUNS:
    ...
    for method, amount in CONFIGS:
        ...
        torch.save(model.state_dict(),
                   f"{OUT}/resnet18_{run_name}_{method}_{int(amount*100)}.pth")
        del model
...
print("\n\n" + "═"*100)
print("  PRUNING SUMMARY — ResNet-18 | SEN1FLOODS11")
print("═"*100)
...
study_df.to_csv(f"{OUT}/resnet18_sen1floods11_pruning_summary.csv",
                index=False)
...
for run_name, (relu_df, filt_df) in inact_frames.items():
    relu_df.to_csv(f"{OUT}/resnet18_{run_name}_dead_activations.csv",
                   index=False)
    filt_df.to_csv(f"{OUT}/resnet18_{run_name}_nearzero_filters.csv",
                   index=False)
print(f"\nSaved → resnet18_sen1floods11_pruning_summary.csv")
...
fig.suptitle("ResNet-18 | SEN1FLOODS11 | Pruning trade-offs "
             "(dot = dense baseline)", fontsize=13, fontweight="bold")
...
plt.tight_layout()
plt.savefig(f"{OUT}/resnet18_sen1floods11_pruning_tradeoff.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → resnet18_sen1floods11_pruning_tradeoff.png")

# dead-activation bar chart (per baseline run)
fig2, axes2 = plt.subplots(1, len(inact_frames), figsize=(16, 5),
                           squeeze=False)
fig2.suptitle("ResNet-18 | dead ReLU activation fraction on val "
...
plt.tight_layout()
plt.savefig(f"{OUT}/resnet18_sen1floods11_dead_activations.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → resnet18_sen1floods11_dead_activations.png")
