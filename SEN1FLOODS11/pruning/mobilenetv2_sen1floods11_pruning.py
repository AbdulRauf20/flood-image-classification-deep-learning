# ╔══════════════════════════════════════════════════════════════════════╗
#  SEN1FLOODS11 — MobileNetV2 LIGHTWEIGHT STUDY: Params, FLOPs & PRUNING
#  Companion to baselines/mobilenetv2_sen1floods11_baselines.py
#
#  Team-lead request:
...
#   3) Diagnoses INACTIVITY: per-layer dead-activation fraction (ReLU6
...
#           MobileNetV2-specific: DEPTHWISE convs (groups == in_channels)
#           are skipped — they hold almost no params and pruning them
#           breaks the channel-per-group structure. Only regular /
#           pointwise 1×1 convs are pruned, which is where nearly all
#           of MobileNetV2's params and FLOPs live.
...
#
#  NOTE ON HEADROOM: MobileNetV2 is already a compact model (~2.2M
#  params) — expect it to tolerate LESS pruning than ResNet-18/VGG-16
#  before F1 drops. That contrast is itself a useful paper result.
# ╚══════════════════════════════════════════════════════════════════════╝
...
def build_mobilenetv2(pretrained=True):
...
def get_baseline(run_name, samples):
    fname = f"mobilenetv2_sen1floods11_{run_name}_best.pth"
    path  = find_weights(fname)
    torch.manual_seed(SEED)
    if torch.cuda.is_available(): torch.cuda.manual_seed_all(SEED)
    model = build_mobilenetv2(pretrained=(path is None)).to(DEVICE)
    if path is not None:
...
def inactivity_report(model, max_batches=8):
    """Fraction of zero activations per ReLU6/ReLU on val data, and
    fraction of conv filters with near-zero L1 norm — the 'inactive'
    parts. MobileNetV2 uses ReLU6 throughout."""
    model = model.to(DEVICE).eval()
    stats, hooks = {}, []
...
def prunable_modules(model):
    """All conv layers + the classifier linear."""
    mods = []
    for name, mod in model.named_modules():
...
def apply_structured(model, amount):
    ...
    MobileNetV2-specific: skip DEPTHWISE convs (groups > 1) — they hold
    almost no params and their channels are tied 1:1 to the preceding
    pointwise conv. Only regular / pointwise (groups == 1) convs are
    pruned; the classifier head is left intact."""
    for name, mod in model.named_modules():
...
for run_name, samples in BASELINE_RUNS:
    ...
    print(f"\n  Inactivity (dead ReLU6 fraction on val, first 15 layers):")
    ...
    for method, amount in CONFIGS:
        ...
        model = build_mobilenetv2(pretrained=False).to(DEVICE)
        ...
        torch.save(model.state_dict(),
                   f"{OUT}/mobilenetv2_{run_name}_{method}_"
...
print("\n\n" + "═"*100)
print("  PRUNING SUMMARY — MobileNetV2 | SEN1FLOODS11")
print("═"*100)
...
study_df.to_csv(f"{OUT}/mobilenetv2_sen1floods11_pruning_summary.csv",
                index=False)
...
for run_name, (relu_df, filt_df) in inact_frames.items():
    relu_df.to_csv(f"{OUT}/mobilenetv2_{run_name}_dead_activations.csv",
                   index=False)
    filt_df.to_csv(f"{OUT}/mobilenetv2_{run_name}_nearzero_filters.csv",
                   index=False)
print(f"\nSaved → mobilenetv2_sen1floods11_pruning_summary.csv")
...
fig.suptitle("MobileNetV2 | SEN1FLOODS11 | Pruning trade-offs "
...
plt.tight_layout()
plt.savefig(f"{OUT}/mobilenetv2_sen1floods11_pruning_tradeoff.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → mobilenetv2_sen1floods11_pruning_tradeoff.png")

# dead-activation bar chart (per baseline run)
fig2, axes2 = plt.subplots(1, len(inact_frames), figsize=(16, 9),
                           squeeze=False)
fig2.suptitle("MobileNetV2 | dead ReLU6 activation fraction on val "
...
plt.tight_layout()
plt.savefig(f"{OUT}/mobilenetv2_sen1floods11_dead_activations.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → mobilenetv2_sen1floods11_dead_activations.png")
