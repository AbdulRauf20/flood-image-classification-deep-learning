...
# ── 13. λ SWEEP ───────────────────────────────────────────────────────
print("\n\n" + "█"*88)
print("  STARTING λ SWEEP  —  MobileNetV2  |  SEN1FLOODS11  (instrumented)")
print("█"*88)
...
cal_df.to_csv(f"{OUT}/mobilenetv2_sen1floods11_calibration_ep{CAL_EPOCH}.csv",
              index=False)
print(f"Saved → mobilenetv2_sen1floods11_calibration_ep{CAL_EPOCH}.csv")
...
fig3.suptitle(
    f"MobileNetV2 | SEN1FLOODS11 | Validation curves per λ — "
...
plt.tight_layout()
plt.savefig(f"{OUT}/mobilenetv2_sen1floods11_valcurves_alpha20.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → mobilenetv2_sen1floods11_valcurves_alpha20.png")

# ── 19. Plot 4 — pseudo-label accuracy & coverage per epoch per λ ─────
fig4, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(16, 6))
fig4.suptitle(
    f"MobileNetV2 | SEN1FLOODS11 | Pseudo-label quality per epoch "
...
plt.tight_layout()
plt.savefig(f"{OUT}/mobilenetv2_sen1floods11_pseudolabel_quality.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → mobilenetv2_sen1floods11_pseudolabel_quality.png")
...
fig5.suptitle(
    f"MobileNetV2 | SEN1FLOODS11 | Calibration at epoch {CAL_EPOCH} "
...
plt.tight_layout()
plt.savefig(f"{OUT}/mobilenetv2_sen1floods11_calibration_ep{CAL_EPOCH}.png",
            dpi=150, bbox_inches="tight")
plt.show()
print(f"Saved → mobilenetv2_sen1floods11_calibration_ep{CAL_EPOCH}.png")
