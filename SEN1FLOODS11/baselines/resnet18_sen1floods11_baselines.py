# ╔══════════════════════════════════════════════════════════════════════╗
#  SEN1FLOODS11 — ResNet-18 SUPERVISED BASELINES
#  Companion to resnet18_sen1floods11_lambda.py (semi-supervised sweep).
#
...
def train_supervised(run_name, samples):
    ...
    print(f"\n{'═'*88}")
    print(f"  ResNet-18  |  {run_name}  |  {len(samples)} train chips  |  "
          f"{E} epochs")
    print(f"{'═'*88}")
...
print("\n\n" + "█"*88)
print("  SUPERVISED BASELINES  —  ResNet-18  |  SEN1FLOODS11")
print("█"*88)
...
all_df.to_csv(f"{OUT}/resnet18_sen1floods11_baselines_history.csv",
              index=False)
summary_df.to_csv(f"{OUT}/resnet18_sen1floods11_baselines_summary.csv",
                  index=False)
for run_name, state in best_states.items():
    torch.save(state, f"{OUT}/resnet18_sen1floods11_{run_name}_best.pth")
print(f"\nSaved → resnet18_sen1floods11_baselines_history.csv")
print(f"Saved → resnet18_sen1floods11_baselines_summary.csv")
print(f"Saved → resnet18_sen1floods11_supervised_only_best.pth")
print(f"Saved → resnet18_sen1floods11_full_supervision_best.pth")

# ── 15. Plot — both baselines' val curves ──────────────────────────────
fig, axes = plt.subplots(2, 3, figsize=(16, 8))
fig.suptitle("ResNet-18 | SEN1FLOODS11 | Supervised baselines "
...
plt.tight_layout()
plt.savefig(f"{OUT}/resnet18_sen1floods11_baselines_curves.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → resnet18_sen1floods11_baselines_curves.png")
