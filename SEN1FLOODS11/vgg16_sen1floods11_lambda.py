...
def train_one_lambda(lam):
    """Train VGG-16 for E epochs with uncertainty offset = lam.
    ...
    print(f"\n{'═'*104}")
    print(f"  VGG-16  |  λ = {lam:.1f}  |  {E} epochs")
    print(f"{'═'*104}")
...
# ── 13. λ SWEEP ───────────────────────────────────────────────────────
print("\n\n" + "█"*88)
print("  STARTING λ SWEEP  —  VGG-16  |  SEN1FLOODS11  (instrumented)")
print("█"*88)
...
all_df.to_csv(f"{OUT}/vgg16_sen1floods11_all_history.csv", index=False)
summary_df.to_csv(f"{OUT}/vgg16_sen1floods11_lambda_summary.csv", index=False)
torch.save(best_states[best_lambda],
           f"{OUT}/vgg16_sen1floods11_best.pth")
print(f"\nSaved → vgg16_sen1floods11_all_history.csv "
      f"(now incl. n_pseudo / pseudo_acc columns)")
print(f"Saved → vgg16_sen1floods11_lambda_summary.csv")
print(f"Saved → vgg16_sen1floods11_best.pth  (λ={best_lambda})")
...
cal_df.to_csv(f"{OUT}/vgg16_sen1floods11_calibration_ep{CAL_EPOCH}.csv",
              index=False)
print(f"Saved → vgg16_sen1floods11_calibration_ep{CAL_EPOCH}.csv")

# ── 16. Plot 1 — metrics vs λ (summary) ──────────────────────────────
fig, axes = plt.subplots(2, 3, figsize=(16, 8))
fig.suptitle("VGG-16 | SEN1FLOODS11 | Best metrics vs λ",
...
plt.tight_layout()
plt.savefig(f"{OUT}/vgg16_sen1floods11_metrics_vs_lambda.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → vgg16_sen1floods11_metrics_vs_lambda.png")
...
fig2.suptitle(
    f"VGG-16 | SEN1FLOODS11 | λ={best_lambda} (best) | Best epoch={best_ep}",
    fontsize=13, fontweight="bold"
)
...
plt.tight_layout()
plt.savefig(f"{OUT}/vgg16_sen1floods11_training_curves.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → vgg16_sen1floods11_training_curves.png")
...
fig3.suptitle(
    f"VGG-16 | SEN1FLOODS11 | Validation curves per λ — "
...
plt.tight_layout()
plt.savefig(f"{OUT}/vgg16_sen1floods11_valcurves_alpha20.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → vgg16_sen1floods11_valcurves_alpha20.png")

# ── 19. Plot 4 — pseudo-label accuracy & coverage per epoch per λ ─────
fig4, (ax_a, ax_b) = plt.subplots(1, 2, figsize=(16, 6))
fig4.suptitle(
    f"VGG-16 | SEN1FLOODS11 | Pseudo-label quality per epoch "
...
plt.tight_layout()
plt.savefig(f"{OUT}/vgg16_sen1floods11_pseudolabel_quality.png",
            dpi=150, bbox_inches="tight")
plt.show()
print("Saved → vgg16_sen1floods11_pseudolabel_quality.png")
...
fig5.suptitle(
    f"VGG-16 | SEN1FLOODS11 | Calibration at epoch {CAL_EPOCH} "
...
plt.tight_layout()
plt.savefig(f"{OUT}/vgg16_sen1floods11_calibration_ep{CAL_EPOCH}.png",
            dpi=150, bbox_inches="tight")
plt.show()
print(f"Saved → vgg16_sen1floods11_calibration_ep{CAL_EPOCH}.png")
